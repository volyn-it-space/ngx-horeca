...
import { inject, Service, PLATFORM_ID, TransferState } from '@angular/core';
...
export class BootstrapService {
	...
	async initialize() {
		...
		if (isPlatformServer(this._platformId)) {
			...
			if (data) {
				...
				this._apply(data);
			} else this._finishFeatureLoading();
	...
	private async _refreshInBrowser() {
		...
		this._finishFeatureLoading();
	}

	private _finishFeatureLoading() {
	...
	private async _load() {
		try {
			return (await firstValueFrom(
				this._httpService.get(`/api/bootstrap/${environment.companyId}`),
			)) as BootstrapData;
		} catch (error) {
			console.error(error);
			return null;
